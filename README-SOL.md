...
* 阅读本章前请先阅读并掌握 [hello-wasm](https://github.com/chainrd/ewasm-rust-demo/blob/master/README.md) 和 [hello-wasm-abi](https://github.com/chainrd/ewasm-rust-demo/blob/master/README-ABI.md) 两章的内容；
...
样例程序 `hello-wasm-sol` 是在 `hello-wasm-abi` 的基础上增加两个方法用来调用 `solidity` 合约，并且增加一个用来描述 `solidity` 合约接口的 `crdabi::Contract` 对象，具体如下：

### abi.rs

使用 `crdabi::Contract` 描述完整的 `hello_wasm_abi` 接口，因为我们要实现这个接口，对于 `hello_sol` 的描述只关注我们要使用的方法 `get/put` 即可
...
pub fn get_sol_contract_abi() -> crdabi::Contract {
...
// 用来调用 solidity 合约 >>>>>>>>>>
let fn_solput = crdabi::Function {
constant: false,
name: String::from("solput"),
inputs: Vec::from(vec![
crdabi::Param { name: String::from("addr"), kind: crdabi::param_type::ParamType::Address },
crdabi::Param { name: String::from("key"), kind: crdabi::param_type::ParamType::String },
crdabi::Param { name: String::from("val"), kind: crdabi::param_type::ParamType::String },
]),
outputs: Vec::default(),
};
let fn_solget = crdabi::Function {
constant: true,
...
```rust
......
fn callsol_put_data(a: &ewasm_api::crdabi::Token, k: &ewasm_api::crdabi::Token, v: &ewasm_api::crdabi::Token) {
...
fn callsol_get_data(a: &ewasm_api::crdabi::Token, k: &ewasm_api::crdabi::Token) -> Vec<u8> {
let addr = a.clone().to_address().expect("error_address");
let gas = ewasm_api::gas_left();
let sol = abi::get_sol_contract_abi();
...
编译并部署 (hello-wasm-sol)[https://github.com/CRDbaap/ewasm-rust-demo/tree/master/hello-wasm-sol] 得到 wasm 合约地址
